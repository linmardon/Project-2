...
//actors spot for /moving/ objects
var actorChars = {
  "@": Player,
...
function Hash(pos) {
    ...
    this.size = new Vector(0.5,1.5);
    this.wobble2 = Math.random() * Math.PI * 3;

}
Hash.prototype.type = 'hash';
...
Level.prototype.animate = function(step, keys) {
  ...
  if (this.stat != null)
    this.visibilityTimer -= thisStep;
   if (this.stat == "visSet" && this.visibilityTimer < 0 )
    {this.stat = null;
    this.player.size = new Vector(1, 1);}
};
...
var wobbleSpeed2 = 3;
var wobbleDist2 = 5;
Hash.prototype.act = function(step){

};
Small.prototype.act = function(step){
};
Invis.prototype.act = function(step){
    this.wobble2 -= step * wobbleSpeed2;
    var wobble2Pos = Math.sin(this.wobble2)
    this.pos = this.basePos.plus(new Vector(0, wobble2Pos));
};
...
Player.prototype.moveY = function(step, level, keys) {
  ...
  if (obstacle) {
    ...
    if (keys.up && this.speed.y > 0)
      ...
      this.speed.y = 0;  
    if (keys.up && level.stat == "big"){
      this.speed.y = -jumpSpeed * 1.2 ;
    }

   if (keys.up && level.stat == "visSet"){
      this.speed.y = +jumpSpeed * 1.2 ;
    }
...
Player.prototype.act = function(step, level, keys) {
  ...
  else if (level.stat == "big") {
    this.size = new Vector(1, 2);
  }

else if (level.stat == "smaller") {
    this.size = new Vector(.5, .5);
  }

else if (level.stat == "visSet") {
    this.size = new Vector(1,1);
}
...
if (type == 'Exit') {
  ...
  else if (type == 'hash' && this.status == null) {
  this.stat = "big";
  }
  else if (type == 'small' && this.status == null) {
  this.stat = "smaller";
 }
  else if (type == 'invis' && this.status == null) {
  this.stat = "visSet";
